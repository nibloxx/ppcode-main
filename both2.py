...
class ComprehensivePropertyReportGenerator:
    ...
    def _generate_market_overview(self, context: str, property_type: str, market_data: Dict) -> str:
        ...
        Make it specific and professional. Use actual data from: {json.dumps(market_data)}
        """
    ...
    def _generate_vacancy_rates(self, context: str, property_type: str, market_data: Dict) -> str:
        ...
        # Format the vacancy rates section
        vacancy_section = f"""**Vacancy Rates *(Based on Public Data & Market Trends)***

-   **Direct Vacancy:** {market_data.get('direct_vacancy', 12.71)}% ({market_data.get('direct_qoq', '+1.02')}% QoQ, {market_data.get('direct_yoy', '+1.68')}% YoY)
...
-   **Total Vacancy:** {market_data.get('total_vacancy', 18.22)}%"""
    ...
    def _generate_lease_rates(self, context: str, property_type: str, market_data: Dict) -> str:
        ...
        lease_section = f"""**Lease Rates *(Based on Public Listings & Industry Trends)***

-   **Overall Average:** ${market_data.get('avg_lease_rate', 24.33)}/SF (${market_data.get('lease_rate_yoy', '-0.04')} YoY)
...
-   **For Developers:** Prioritize **medical office projects** and mixed-use spaces to align with evolving work trends."""
    ...
    def _generate_data_sources(self) -> str:
        ...
        sources = f"""*This report is based on publicly available data from multiple sources, including government property records, industry reports, and market analysis. While we have conducted an independent assessment, all figures should be verified with authoritative sources before making investment decisions. This document is for informational purposes only and should not be considered financial or investment advice.*
...
6.  **National Market Insights** -- Hybrid Work and Office Space Adjustments"""
    ...
    def create_word_document(self, property_data: PropertyReportData) -> str:
        ...
        # Add market analysis section at the end
