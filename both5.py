...
6.  National Market Insights – Hybrid Work and Office Space Adjustments"""
    ...
    def _replace_text_in_xml(self, xml_content: str, replacements: Dict[str, str]) -> str:
    ...
    def _process_document_xml(self, doc_path: str, replacements: Dict[str, str], output_path: str):
        """Process document XML files to replace text in all locations including text boxes"""
        ...
        try:
            ...
            # Process all XML files that might contain text
            xml_files = [
            ...
            # Also process all drawing files which contain text boxes
            for root, dirs, files in os.walk(temp_dir / 'word'):
                for file in files:
                    if file.endswith('.xml'):
                        file_path = os.path.join(root, file)
                        rel_path = os.path.relpath(file_path, temp_dir)
                        if 'drawing' in file or 'charts' in file:
                            xml_files.append(rel_path)
            
            # Process each XML file
            for xml_file_pattern in xml_files:
                # Handle wildcard patterns
                if '*' in xml_file_pattern:
                    import glob
                    matching_files = glob.glob(str(temp_dir / xml_file_pattern))
                    files_to_process = [os.path.relpath(f, temp_dir) for f in matching_files]
                else:
                    files_to_process = [xml_file_pattern] if (temp_dir / xml_file_pattern).exists() else []
                ...
                for xml_file in files_to_process:
                    ...
                    if xml_path.exists():
                        ...
                        modified_content = self._replace_text_in_xml(xml_content, replacements)
                        
                        # Write back if changed
                        if modified_content != xml_content:
                            with open(xml_path, 'w', encoding='utf-8') as f:
                                f.write(modified_content)
                            logger.info(f"Replaced text in {xml_file}")
            
            # Re-zip the files to create the new DOCX
    ...
    def _replace_text_in_document(self, doc: Document, replacements: Dict[str, str]):
        ...
        # Replace in tables with better error handling
        try:
            for table_idx, table in enumerate(doc.tables):
                try:
                    # Process cells directly without iterating through rows first
                    # This handles merged cells better
                    processed_cells = set()
                    for row_idx, row in enumerate(table.rows):
                        for cell_idx, cell in enumerate(row.cells):
                            ...
                            try:
                                ...
                                logger.warning(f"Error processing cell at table {table_idx}, row {row_idx}, cell {cell_idx}: {e}")
                except Exception as e:
        ...
        try:
            for section_idx, section in enumerate(doc.sections):
                ...
                try:
                    ...
                    logger.warning(f"Error processing header in section {section_idx}: {e}")
                ...
                try:
                    ...
                    logger.warning(f"Error processing footer in section {section_idx}: {e}")
        except Exception as e:
            logger.warning(f"Error processing sections: {e}")
    ...
    def create_word_document(self, property_data: PropertyReportData) -> str:
        ...
        try:
            ...
            try:
                # Sometimes helps with problematic documents
                import warnings
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    doc = Document(self.template_path)
            except Exception as e2:
                raise Exception(f"Failed to load template document: {e2}")
        ...
        # First, try standard replacement through python-docx
        try:
            self._replace_text_in_document(doc, replacements)
        except Exception as e:
            logger.error(f"Error during text replacement: {e}")
            # Continue anyway - partial replacement is better than none
        ...
        # Save document first with standard method
        ...
        # Now process the saved document at XML level to catch text boxes
        try:
            self._process_document_xml(str(temp_path), replacements, str(output_path))
            ...
            # If XML processing fails, use the temp file as output
            if temp_path.exists():
                import shutil
