...
6.  National Market Insights – Hybrid Work and Office Space Adjustments"""
    ...
    def _process_textboxes_in_docx(self, docx_path: str, replacements: Dict[str, str]) -> bool:
        """
        Process text boxes in a saved Word document with support for multiple replacements
        ...
        """
        temp_dir = Path("temp_textbox_processing")
        ...
        try:
            ...
            # Process main document and all subdirectories
            for xml_file in word_dir.rglob('*.xml'):
                # Skip theme, styles, and settings files
                if any(skip in xml_file.name.lower() for skip in ['theme', 'styles', 'settings', 'websettings', 'fonttable', 'app.xml', 'core.xml']):
                    continue
                ...
                try:
                    ...
                    # Process the XML content with proper text box handling
                    content = self._process_xml_content_for_textboxes(content, replacements)
                    
                    # Write back if changed
                    if content != original_content:
                        with open(xml_file, 'w', encoding='utf-8') as f:
                            f.write(content)
                        logger.info(f"Updated text in {xml_file.name}")
                        files_processed += 1
                        
                except Exception as e:
                    logger.warning(f"Error processing {xml_file.name}: {e}")
            
            # Re-zip the files back to the original docx
            ...
            logger.info(f"Text box processing complete. Files updated: {files_processed}")
            return True
            
        except Exception as e:
            logger.error(f"Error processing text boxes: {e}")
            return False
    ...
    def _process_xml_content_for_textboxes(self, xml_content: str, replacements: Dict[str, str]) -> str:
        """
        Process XML content to handle text boxes with multiple replacements
        """
        # First, do simple string replacement for all placeholders
        # This handles cases where placeholders are in single text elements
        for placeholder, replacement in replacements.items():
            replacement_text = replacement.replace('--', '–')
            xml_content = xml_content.replace(placeholder, replacement_text)
        
        # Now handle cases where placeholders might be split across multiple text elements
        # This is common in text boxes
        try:
            # Parse the XML
            ...
            # Different types of text box containers
            textbox_paths = [
                './/w:txbxContent',
                './/wps:txbx',
                './/v:textbox'
            ...
            for path in textbox_paths:
                textbox_elements.extend(root.findall(path, namespaces))
            
            # Process each text box
            for textbox in textbox_elements:
                self._process_textbox_element(textbox, replacements, namespaces)
            
            # Convert back to string
            xml_content = ET.tostring(root, encoding='unicode', method='xml')
            
        except Exception as e:
            logger.debug(f"XML parsing for advanced text box processing failed: {e}")
            # The simple replacement already happened, so this is still useful
        
        return xml_content

    def _process_textbox_element(self, textbox_element, replacements: Dict[str, str], namespaces: Dict[str, str]):
        """
        Process a single text box element to handle multiple replacements
        """
        # Get all text elements within this text box
        text_elements = textbox_element.findall('.//w:t', namespaces)
        
        if not text_elements:
            return
        
        # Combine all text to check for split placeholders
        combined_text = ''.join(elem.text or '' for elem in text_elements)
        
        # Check if any placeholders exist in the combined text
        has_placeholders = any(placeholder in combined_text for placeholder in replacements.keys())
        ...
        for placeholder, replacement in replacements.items():
            ...
            # Find where the placeholder starts in the combined text
            placeholder_start = combined_text.find(placeholder)
            placeholder_end = placeholder_start + len(placeholder)
            
            # Track position as we iterate through text elements
            current_pos = 0
            ...
            for i, elem in enumerate(text_elements):
                ...
                # Check if placeholder starts in this element
                if elem_start <= placeholder_start < elem_end:
                    # Placeholder starts in this element
                    if placeholder_end <= elem_end:
                        # Entire placeholder is in this element
                        elem.text = elem_text.replace(placeholder, replacement_text)
                        placeholder_handled = True
                    else:
                        # Placeholder spans multiple elements
                        # Replace the start part
                        start_offset = placeholder_start - elem_start
                        elem.text = elem_text[:start_offset] + replacement_text
                        
                        # Clear subsequent elements until we've covered the placeholder
                        j = i + 1
                        remaining_length = placeholder_end - elem_end
                        
                        while j < len(text_elements) and remaining_length > 0:
                            next_elem = text_elements[j]
                            next_text = next_elem.text or ''
                            
                            if len(next_text) >= remaining_length:
                                # This element contains the end of the placeholder
                                next_elem.text = next_text[remaining_length:]
                                remaining_length = 0
                            else:
                                # Clear this entire element
                                next_elem.text = ''
                                remaining_length -= len(next_text)
            ...
            # Update combined text for next placeholder
            combined_text = ''.join(elem.text or '' for elem in text_elements)
    ...
    def create_word_document(self, property_data: PropertyReportData) -> str:
        ...
        # Process text boxes in the saved document
        logger.info("Processing text boxes...")
        if self._process_textboxes_in_docx(str(output_path), replacements):
            logger.info("Text box processing complete")
        else:
            logger.warning("Text box processing encountered issues, but document was saved")
